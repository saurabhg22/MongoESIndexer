...
describe("setup", function () {
    it("should fail with invalid configDir", async function () {
        mongoESIndexer = new MongoESIndexer(
            "./invalidpath",
            "http://localhost:9200/",
    ...
    it("should pass with correct configDir", async function () {
        mongoESIndexer = new MongoESIndexer(
            "./test/testconfigs",
            "http://elastic:radiant@localhost:9200/",
            "mongodb://localhost:27017/testdb",
            "testdb"
        );
...
describe("indexOne", () => {
    it("should index single instance", async function () {
        await mongoESIndexer.indexOne("testdbuser", users[0]._id).should.be
            .fulfilled;
    });
});
...
describe("updateOne", () => {
    it("should update single instance", async function () {
        await mongoESIndexer.updateOne("testdbuser", users[0]._id, {
            firstName: "firstName",
        }).should.be.fulfilled;
    });
});
...
describe("deleteOne", () => {
    it("should delete single instance", async function () {
        await mongoESIndexer.deleteOne("testdbuser", users[0]._id).should.be
            .fulfilled;
    });
    it("should reject delete single instance with invalid id", async function () {
        await mongoESIndexer.deleteOne("testdbuser", "invalid id").should.be
            .rejected;
    });
});
...
describe("bulkIndex", () => {
    it("should index 300 users", async function () {
        await mongoESIndexer.bulkIndex("testdbuser", { limit: 300 }).should.be
            .fulfilled;
    });
});
...
describe("bulkUpdate", () => {
    it("should update age to 20 of all users", async function () {
        await mongoESIndexer.bulkUpdate("testdbuser", {}, { Age: 20 }).should.be
            .fulfilled;
    });
});
